...
- 在实例列表或详情直接选择单个数据库的启动、停止或重启，两个入口都会在提交前确认实例状态和
  连接中断影响；也可批量执行，并原地跟踪每个任务到成功、失败和安全恢复入口。
- 工作台汇总可用主机、运行实例、进行中任务、未处理告警、仍待恢复的失败任务、异常实例和
  即将到期实例；创建时记录用途、负责人和预计到期时间，默认负责人取当前账号、到期时间为
  7 天，提醒但不自动停删实例。到期队列集中显示每个候选的备份、进行中操作、状态阻断和缺失
  清理上下文，可筛选并批量延期或改为长期保留；提交前逐项预览，部分失败可只重试失败项。
  永久删除不进入批量入口，仍逐实例审查并输入精确名称确认。
- 主机相关任务失败时，从工作台、任务详情或实例详情进入主机检查会保留原任务上下文；
  主机重新检测恢复在线后可在同一处重试，并返回实例确认状态和连接信息。
- 为单机实例创建可校验的主机本地冷备份；备份列表在桌面和平板宽度持续显示恢复与删除入口，
  创建、恢复或删除请求未入队时保留原因、影响和重新确认入口；支持按时区配置每日/每周自动备份
  和保留份数，恢复失败时自动回滚到操作前数据。
- 控制平面可一致备份和恢复 PostgreSQL 元数据、主密钥及上传制品；支持加密归档、校验、
  恢复前安全快照与失败自动回滚，升级前默认创建恢复点。
- 运行配置变更会重新校验主机容量和 Compose；失败时恢复旧配置及原运行/停止状态。
- 内置常见数据库模板；创建时会按团队历史部署次数和最近使用时间展示最多 3 个常用固定版本，
  仍可搜索完整目录。资源配置与部署主机在同一步完成，候选主机会按当前预留、资源和端口即时
  标明部署后余量或不可选原因。也可上传 Compose 模板包和 Docker 离线镜像，并扫描、人工清理
  长期未使用的控制端镜像归档。
- 管理员和运维可从实例列表直接复制已有部署配置，不必先进入详情；名称、密码、主机、端口和
  新的交付责任仍要求重新确认。
- 创建提交后停留在实例详情持续展示部署阶段、进度和下一步；当前页会说明下一项平台自动动作，
  并允许管理员和运维在安全阶段原地取消部署；进入任务中心仍保留同一工作上下文，
  部署成功后管理员和运维可直接进入连接信息交付，只读角色只查看数据库状态。交付页按不可变模板
  声明展示实际认证方式；未启用密码认证的模板不会生成或交付无效密码，并会提示隔离测试网络风险。
- 主机与容器监控、日志、告警、通用 Webhook、任务中心和永久审计记录。
- 在线与完全离线的 Docker Compose 安装方式。

首版不包含 Windows、跨主机或对象存储备份、备份下载、多主机数据库集群、SQL 控制台和公开 API
令牌。
