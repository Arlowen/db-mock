...
test('initializes the platform and switches the embedded interface language', async ({ page, browser }) => {
  ...
  await templateDetails.locator('.ant-modal-footer').getByRole('button', { name: /关\s*闭/ }).click()
  ...
  await page.locator('.page-header').getByRole('button', { name: '创建' }).click()
  const createUserDialog = page.getByRole('dialog', { name: '创建' })
  ...
  await developerDialog.getByRole('switch', { name: '已禁用' }).click()
  await developerDialog.getByRole('button', { name: /保\s*存/ }).click()
  await expect(developerDialog).toBeHidden()
