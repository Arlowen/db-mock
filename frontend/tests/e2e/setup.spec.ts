...
test('initializes the platform and switches the embedded interface language', async ({ page, browser }, testInfo) => {
  ...
  await projectDialog.getByRole('combobox', { name: '默认模板 / 版本' }).click()
  ...
  await expect(projectDialog.getByRole('combobox', { name: '默认模板 / 版本' })).toHaveText(/PostgreSQL 17/)
  await expect(projectDialog.getByLabel('CPU')).toHaveValue('2.00')
  await expect(projectDialog.getByLabel('内存 GiB')).toHaveValue('4')
  await expect(projectDialog.getByLabel('磁盘 GiB')).toHaveValue('20')
  await projectDialog.getByRole('button', { name: '关闭', exact: true }).click()
  await page.getByRole('button', { name: '创建项目' }).click()
  ...
  await expect(projectDefaultsDrawer.getByLabel('CPU')).toHaveValue(/^2(?:\.00)?$/)
  ...
  await projectDefaultsDrawer.getByLabel('CPU').fill('3')
  ...
  await createInstanceDrawer.getByRole('button', { name: '下一步' }).click()
  await expect(createInstanceDrawer.getByLabel('CPU')).toHaveValue(/^3(?:\.00)?$/)
