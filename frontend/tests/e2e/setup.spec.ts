...
test('initializes the platform and switches the embedded interface language', async ({ page, browser }) => {
  ...
  await expect(accountDialog.getByLabel('显示名称')).toHaveValue('E2E Developer')
  await accountDialog.getByLabel('显示名称').fill('E2E Developer Self')
  await accountDialog.getByRole('button', { name: '保存个人资料' }).click()
  await expect(developerPage.getByText('个人资料已保存')).toBeVisible()
  await accountDialog.getByLabel('显示名称').fill('E2E Developer')
  await accountDialog.getByRole('button', { name: '保存个人资料' }).click()
  ...
  await expect(developerPage).toHaveURL(/\/$/)
  await expect(developerPage.getByText('总览', { exact: true }).first()).toBeVisible()
