...
export function SettingsPage() {
  ...
  return <><PageHeader title={t('settings')} description="Runtime policy values stored in PostgreSQL. Process-level networking and TLS remain environment variables." /><Row gutter={[16, 16]}>{Object.entries(items).map(([key, value]) => <Col xs={24} lg={12} key={key}><Card title={<Space><CodeOutlined />{key}</Space>} extra={<Button type="text" icon={<EditOutlined />} onClick={() => show(key)}>{t('edit')}</Button>}><pre className="settings-json">{JSON.stringify(value, null, 2)}</pre></Card></Col>)}</Row>
    <Card className="configuration-note"><Typography.Title level={4}>Deployment environment</Typography.Title><Typography.Paragraph type="secondary">Listen address, PostgreSQL URL, master key, artifact directory, public URL, optional TLS certificate/key, session duration and the 50 GiB upload ceiling are configured with DBMOCK_* environment variables.</Typography.Paragraph></Card>
