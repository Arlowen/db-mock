...
export function ProjectsPage() {
  ...
  return <><PageHeader title={t('projects')} description="Logical grouping only — all users keep equal access." actions={<Button type="primary" icon={<PlusOutlined />} onClick={() => show()}>{t('create')}</Button>} />
    {items.length === 0 ? <Card><EmptyState action={() => show()} /></Card> : <Row gutter={[16, 16]}>{items.map((item) => <Col xs={24} md={12} xl={8} key={item.id}><Card className="project-card" actions={[<EditOutlined key="edit" onClick={() => show(item)} />, <Popconfirm key="delete" title={t('delete')} description="Projects containing hosts or instances cannot be deleted." onConfirm={() => void remove(item)}><DeleteOutlined /></Popconfirm>]}><Space direction="vertical"><Space><span className="project-dot" style={{ background: item.color }} /><Typography.Title level={4}>{item.name}</Typography.Title></Space><Typography.Paragraph type="secondary">{item.description || '—'}</Typography.Paragraph></Space></Card></Col>)}</Row>}
    <Modal title={editing ? t('edit') : t('create')} open={open} onCancel={() => setOpen(false)} onOk={() => void submit()} destroyOnHidden><Form form={form} layout="vertical"><Form.Item name="name" label={t('name')} rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="description" label={t('description')}><Input.TextArea rows={3} /></Form.Item><Form.Item name="color" label="Color"><Input type="color" /></Form.Item></Form></Modal>
  </>
}
