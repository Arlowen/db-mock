...
export function ProjectsPage() {
  const { t } = useTranslation(); const { message, modal } = App.useApp(); const [items, setItems] = useState<Project[]>([]); const [loading, setLoading] = useState(true); const [loadError, setLoadError] = useState(''); const [saveError, setSaveError] = useState(''); const [saving, setSaving] = useState(false); const [draftDirty, setDraftDirty] = useState(false); const [open, setOpen] = useState(false); const [editing, setEditing] = useState<Project | null>(null); const [form] = Form.useForm<ProjectForm>(); const draftBaseline = useRef<ProjectForm | null>(null)
  ...
  const submit = async () => {
    try {
      ...
      await api(editing ? `/projects/${editing.id}` : '/projects', { method: editing ? 'PUT' : 'POST', body: values })
  ...
  return <><PageHeader title={t('projects')} description={t('projectsDescription')} />
    {loadError && <Alert className="instance-page-alert" type={items.length ? 'warning' : 'error'} showIcon message={t('projectListLoadFailed')} description={loadError} action={<Button size="small" loading={loading} onClick={() => { setLoading(true); void load() }}>{t('retry')}</Button>} />}
    ...
    <Modal title={editing ? t('editProject') : t('createProject')} open={open} onCancel={closeEditor} onOk={() => void submit()} confirmLoading={saving} closable={!saving} maskClosable={!saving} cancelButtonProps={{ disabled: saving }} okButtonProps={{ disabled: !projectName?.trim() || (!!editing && !draftDirty) }} okText={t('save')} destroyOnHidden><Form form={form} layout="vertical" onValuesChange={(_, values) => { setSaveError(''); setDraftDirty(projectDraftChanged(values, draftBaseline.current)) }}>{saveError && <Alert className="form-save-alert" type="error" showIcon message={t('projectSaveFailed')} description={saveError} />}<Form.Item name="name" label={t('name')} rules={[{ required: true }]}><Input autoFocus /></Form.Item><Form.Item name="description" label={t('description')}><Input.TextArea rows={3} /></Form.Item><Form.Item name="color" label={t('color')} getValueFromEvent={(_, hex: string) => hex}><ColorPicker format="hex" showText={(color) => color.toHexString()} /></Form.Item></Form></Modal>
  </>
}
