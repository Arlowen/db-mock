...
export function ProjectsPage() {
  ...
  return <><PageHeader title={t('projects')} description={t('projectsDescription')} actions={<Button type="primary" icon={<PlusOutlined />} onClick={() => show()}>{t('create')}</Button>} />
    {items.length === 0 ? <Card loading={loading}><EmptyState action={() => show()} actionLabel={t('create')} description={t('projectsEmptyDescription')} /></Card> : <Row gutter={[16, 16]}>{items.map((item) => <Col xs={24} md={12} xl={8} key={item.id}><Card className="project-card" style={{ borderTopColor: item.color }} actions={[<Button key="edit" type="text" aria-label={`${t('edit')} ${item.name}`} title={t('edit')} icon={<EditOutlined />} onClick={() => show(item)} />, <Popconfirm key="delete" title={t('deleteProjectConfirm', { name: item.name })} description={t('deleteProjectHint')} okText={t('delete')} cancelText={t('cancel')} okButtonProps={{ danger: true }} onConfirm={() => void remove(item)}><Button type="text" danger aria-label={`${t('delete')} ${item.name}`} title={t('delete')} icon={<DeleteOutlined />} /></Popconfirm>]}><div className="project-card-heading"><span className="project-dot" style={{ background: item.color }} /><Typography.Title level={4}>{item.name}</Typography.Title></div><Typography.Paragraph className="project-card-description" type="secondary">{item.description || t('noDescription')}</Typography.Paragraph></Card></Col>)}</Row>}
