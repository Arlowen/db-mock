...
export function HostsPage() {
  ...
  const filteredItems = useMemo(() => projectFilter ? items.filter((item) => item.projectId === projectFilter) : items, [items, projectFilter])
  ...
  return <><PageHeader title={t('hosts')} description={t('hostsDescription')} />
    ...
    {(items.length > 0 || !loadError) && <Card className="host-table-card"><div className="embedded-toolbar"><div className="host-list-heading"><Typography.Text strong>{t('hosts')}</Typography.Text><Typography.Text type="secondary">{t(projectFilter ? 'hostFilteredResultCount' : 'hostResultCount', { filtered: filteredItems.length, total: items.length, count: items.length })}</Typography.Text></div><Space wrap><Select className="host-project-filter" aria-label={t('project')} value={projectFilter} onChange={setProjectFilter} options={[{ value: '', label: t('allProjects') }, ...projects.map((project) => ({ value: project.id, label: project.name }))]} /><Button loading={loading} icon={<ReloadOutlined />} onClick={() => { setLoading(true); void load() }}>{t('refresh')}</Button>{canOperate && items.length > 0 && <Button type="primary" icon={<PlusOutlined />} onClick={() => show()}>{t('addHost')}</Button>}</Space></div><Table rowKey="id" loading={loading} dataSource={filteredItems} columns={columns} pagination={false} tableLayout="fixed" scroll={{ x: 989 }} locale={{ emptyText: <EmptyState compact action={projectFilter ? () => setProjectFilter('') : canOperate ? () => show() : undefined} actionLabel={projectFilter ? t('clearFilters') : canOperate ? t('addHost') : undefined} description={t(projectFilter ? 'hostsFilteredEmptyDescription' : 'noHostsDescription')} /> }} /></Card>}
