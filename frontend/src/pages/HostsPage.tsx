...
export function HostsPage() {
  ...
  const show = (item?: Host) => { if (item) setDetail(null); form.resetFields(); setEditing(item ?? null); setEditorDirty(false); setFingerprint(item?.hostKey ?? ''); setProbe(null); setVerificationDirty(false); form.setFieldsValue(item ? { ...item, credential: '', passphrase: '' } : { sshPort: 22, authType: 'private_key', dataRoot: '/opt/dbmock', portStart: 20000, portEnd: 40000, manageDocker: false, maintenance: false, autoRestartDefault: true }); setOpen(true) }
  ...
  const invalidateVerification = (changed: Partial<HostForm>) => {
    ...
    const connectionChanged = Object.keys(changed).some((key) => verificationFields.has(key))
    ...
    setFingerprint(''); setProbe(null); setVerificationDirty(true)
  }
