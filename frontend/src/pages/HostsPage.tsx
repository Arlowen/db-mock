...
export function HostsPage() {
  ...
  return <><PageHeader title={t('hosts')} description={t('hostsDescription')} />
    ...
    <Card className="host-table-card"><div className="embedded-toolbar"><Typography.Text strong>{t('hosts')}</Typography.Text><Space wrap><Button icon={<ReloadOutlined />} onClick={() => void load()}>{t('refresh')}</Button>{canOperate && <Button type="primary" icon={<PlusOutlined />} onClick={() => show()}>{t('addHost')}</Button>}</Space></div><Table rowKey="id" loading={loading} dataSource={items} columns={columns} pagination={false} tableLayout="fixed" locale={{ emptyText: <EmptyState compact action={canOperate ? () => show() : undefined} actionLabel={canOperate ? t('addHost') : undefined} description={t('noHostsDescription')} /> }} /></Card>
