...
export function HostsPage() {
  ...
  return <><PageHeader title={t('hosts')} description={t('hostsDescription')} actions={<><Button icon={<ReloadOutlined />} onClick={() => void load()}>{t('refresh')}</Button>{canOperate && <Button type="primary" icon={<PlusOutlined />} onClick={() => show()}>{t('addHost')}</Button>}</>} />
    ...
    <Card className="host-table-card"><Table rowKey="id" loading={loading} dataSource={items} columns={columns} pagination={false} tableLayout="fixed" locale={{ emptyText: <EmptyState compact action={canOperate ? () => show() : undefined} actionLabel={canOperate ? t('addHost') : undefined} description={t('noHostsDescription')} /> }} /></Card>
