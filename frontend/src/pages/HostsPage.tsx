import { ArrowLeftOutlined, ArrowRightOutlined, CloudServerOutlined, DatabaseOutlined, DeleteOutlined, EditOutlined, PlusOutlined, RedoOutlined, ReloadOutlined, SafetyCertificateOutlined, SearchOutlined } from '@ant-design/icons'
...
export function HostsPage() {
  ...
  const columns = useMemo(() => [
    ...
    { title: t('actions'), width: 84, align: 'right' as const, render: (_: unknown, item: Host) => canOperate ? <Space size={4}><Button type="text" size="small" aria-label={`${t('reprobeHost')} ${item.name}`} title={t('reprobeHost')} icon={<ReloadOutlined />} loading={actioning === 'probe'} disabled={!!actioning && actioning !== 'probe'} onClick={() => void action(item, 'probe')} /><Button type="text" size="small" aria-label={`${t('edit')} ${item.name}`} title={t('edit')} icon={<EditOutlined />} disabled={!!actioning} onClick={() => show(item)} /></Space> : null },
