...
import { Alert, App, Button, Card, Collapse, Descriptions, Drawer, Dropdown, Form, Input, InputNumber, Modal, Popconfirm, Progress, Select, Space, Steps, Switch, Table, Tag, Typography } from 'antd'
...
import { dockerManagementReady } from '../lib/host-verification'
...
export function HostsPage() {
  const { t, i18n } = useTranslation(); const { timezone } = useSystemSettings(); const { message, modal } = App.useApp(); const navigate = useNavigate(); const notifyTask = useTaskNotification(); const [params, setParams] = useSearchParams(); const hostID = params.get('host'); const returnTo = safeCreateReturnPath(params.get('returnTo')); const [items, setItems] = useState<Host[]>([]); const [projects, setProjects] = useState<Project[]>([]); const [instances, setInstances] = useState<Instance[]>([]); const [hostTasks, setHostTasks] = useState<Task[]>([]); const [detailError, setDetailError] = useState(''); const [open, setOpen] = useState(false); const [detail, setDetail] = useState<Host | null>(null); const [editing, setEditing] = useState<Host | null>(null); const [editorDirty, setEditorDirty] = useState(false); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [testing, setTesting] = useState(false); const [actioning, setActioning] = useState(''); const [fingerprint, setFingerprint] = useState(''); const [probe, setProbe] = useState<HostProbeResult | null>(null); const [verificationDirty, setVerificationDirty] = useState(false); const [verificationReason, setVerificationReason] = useState<VerificationReason>(''); const verificationSection = useRef<HTMLDivElement>(null); const [form] = Form.useForm<HostForm>()
  ...
  const show = (item?: Host) => { if (item) setDetail(null); form.resetFields(); setEditing(item ?? null); setEditorDirty(false); setFingerprint(item?.hostKey ?? ''); setProbe(null); setVerificationDirty(false); setVerificationReason(''); form.setFieldsValue(item ? { ...item, credential: '', passphrase: '' } : { sshPort: 22, authType: 'private_key', dataRoot: '/opt/dbmock', portStart: 20000, portEnd: 40000, manageDocker: false, maintenance: false, autoRestartDefault: true }); setOpen(true) }
  ...
  const invalidateVerification = (changed: Partial<HostForm>) => {
    ...
    if (changed.manageDocker === false && verificationReason === 'docker_policy' && editing) {
      setFingerprint(editing.hostKey ?? ''); setProbe(null); setVerificationDirty(false); setVerificationReason('')
      return
    }
    ...
    setFingerprint(''); setProbe(null); setVerificationDirty(true); setVerificationReason(reason)
  }
  ...
  const operationPanel = operationTask && <div className={`instance-operation host-operation is-${activeTask ? 'active' : 'failed'}`}>
    ...
    <Space className="instance-operation-actions">{failedTask && !activeTask && <Button type="primary" icon={<ReloadOutlined />} loading={actioning === 'retry-task'} disabled={!!actioning && actioning !== 'retry-task'} onClick={() => void retryTask()}>{t('retryTask')}</Button>}<Button onClick={() => navigate(`/tasks?task=${operationTask.id}`)}>{t('viewTask')}</Button></Space>
  </div>
  ...
  return <><PageHeader title={t('hosts')} description={t('hostsDescription')} actions={<><Button icon={<ReloadOutlined />} onClick={() => void load()}>{t('refresh')}</Button><Button type="primary" icon={<PlusOutlined />} onClick={() => show()}>{t('addHost')}</Button></>} />
    ...
    <Modal title={editing ? t('edit') : t('addHost')} open={open} onCancel={closeEditor} width={760} style={{ top: 32 }} styles={{ body: { maxHeight: 'calc(100vh - 160px)', overflowY: 'auto', paddingRight: 4 } }} destroyOnHidden footer={<div className="workflow-modal-footer"><Button disabled={saving || testing} onClick={closeEditor}>{t('cancel')}</Button><Space>{(!editing || verificationDirty || !fingerprint) && <Button loading={testing} disabled={saving} icon={<SafetyCertificateOutlined />} onClick={() => void test()}>{t('testConnection')}</Button>}<Button type="primary" loading={saving} disabled={testing || !fingerprint || !dockerPolicyReady} onClick={() => void submit()}>{t('save')}</Button></Space></div>}>
