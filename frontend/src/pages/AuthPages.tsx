  ...
  return <div className="auth-shell">
    <Card className="auth-card" bordered={false}>
      <Space direction="vertical" size={24} style={{ width: '100%' }}>
        <div className="brand-lockup"><span className="brand-mark"><DatabaseOutlined /></span><div><Typography.Title level={2}>DB Mock</Typography.Title><Typography.Text type="secondary">Docker Compose database control plane</Typography.Text></div></div>
        ...
        <Form form={form} layout="vertical" onFinish={submit} initialValues={{ locale: i18n.language }} requiredMark={false}>
          ...
          {setup && <Form.Item label={t('language')} name="locale"><Select size="large" options={[{ value: 'zh-CN', label: '简体中文' }, { value: 'en-US', label: 'English' }]} /></Form.Item>}
          <Button htmlType="submit" type="primary" size="large" block>{setup ? t('initialize') : t('login')}</Button>
        </Form>
        <Button type="text" icon={<GlobalOutlined />} onClick={() => { const locale = i18n.language === 'zh-CN' ? 'en-US' : 'zh-CN'; void i18n.changeLanguage(locale); localStorage.setItem('dbmock-locale', locale) }}>{i18n.language === 'zh-CN' ? 'English' : '简体中文'}</Button>
      </Space>
    </Card>
  </div>
}
