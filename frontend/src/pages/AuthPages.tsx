...
import { applyLocale, oppositeLocale, type AppLocale } from '../lib/locale'
...
export function AuthPage({ setup }: { setup: boolean }) {
  ...
  const switchLanguage = async () => {
    ...
    if (setup) form.setFieldValue('locale', targetLocale)
  }
    ...
    <Card className="auth-card" bordered={false}>
      <Space direction="vertical" size={24} style={{ width: '100%' }}>
        ...
        <Form form={form} layout="vertical" onFinish={submit} initialValues={{ locale: i18n.language }} requiredMark={false}>
          ...
          {setup && <Form.Item label={t('language')} name="locale"><Select size="large" onChange={(locale) => void applyLocale(locale as AppLocale)} options={[{ value: 'zh-CN', label: t('languageChinese') }, { value: 'en-US', label: t('languageEnglish') }]} /></Form.Item>}
