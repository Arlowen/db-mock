...
export function AlertsPage() {
  ...
  const alertTab = <Card className="alert-list-card">
    ...
    ]} /><Button className="alert-refresh-button" aria-label={t('refresh')} title={t('refresh')} icon={<ReloadOutlined />} loading={loading} onClick={() => void load()}>{compactDeliveryTable ? null : t('refresh')}</Button></div>
  ...
  const webhookTab = <Card
    ...
    loading={loading}
    title={<Space size={8}>{t('webhook')}<Tag>{webhooks.length}</Tag></Space>}
    extra={<Space className="webhook-section-actions">
      <Button aria-label={t('refresh')} title={t('refresh')} icon={<ReloadOutlined />} loading={loading} onClick={() => void load()}>{compactDeliveryTable ? null : t('refresh')}</Button>
      {canOperate && <Button type="primary" aria-label={t('addWebhook')} title={t('addWebhook')} icon={<PlusOutlined />} onClick={showCreateWebhook}>{compactDeliveryTable ? null : t('addWebhook')}</Button>}
    </Space>}
  >
  ...
  return <>
    ...
    <Tabs className="alert-page-tabs" activeKey={activeTab} onChange={(tab) => setQuery({ tab: tab === 'webhooks' ? 'webhooks' : undefined, alert: undefined, webhook: undefined })} items={[
