...
export function UsersPage() {
  ...
  const save = async () => { try { setSaving(true); const values = await form.validateFields(); const body = editing ? { displayName: values.displayName, locale: values.locale, role: editing.id === currentUser?.id ? undefined : values.role, password: values.password || '', disabled: !!values.disabled } : { username: values.username, displayName: values.displayName, locale: values.locale, role: values.role, password: values.password }; await api(editing ? `/users/${editing.id}` : '/users', { method: editing ? 'PATCH' : 'POST', body }); if (editing?.id === currentUser?.id) await reloadCurrentUser(); message.success(t('saved')); finishCloseEditor(); await load() } catch (error) { if (error instanceof Error) message.error(errorMessage(error)) } finally { setSaving(false) } }
  return <><PageHeader title={t('users')} description={t('usersDescription')} /><Card title={t('users')} extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => show()}>{t('create')}</Button>}><Table rowKey="id" loading={loading} dataSource={items} pagination={false} columns={[{ title: t('username'), dataIndex: 'username', render: (value: string, item: User) => <Space><span className="user-avatar">{item.displayName.slice(0, 1).toUpperCase()}</span><div><Space size={6} wrap><Typography.Text strong>{item.displayName}</Typography.Text>{item.id === currentUser?.id && <Tag color="blue">{t('currentAccount')}</Tag>}</Space><br /><Typography.Text type="secondary">{value}</Typography.Text></div></Space> },{ title: t('role'), dataIndex: 'role', render: (value: UserRole) => <Tag color={value === 'admin' ? 'blue' : value === 'operator' ? 'cyan' : undefined}>{t(`role_${value}`)}</Tag> },{ title: t('language'), dataIndex: 'locale', render: (value: string) => <Tag>{value === 'zh-CN' ? t('languageChinese') : t('languageEnglish')}</Tag> },{ title: t('status'), render: (_: unknown, item: User) => <Tag color={item.disabledAt ? 'default' : 'green'}>{item.disabledAt ? t('disabled') : t('active')}</Tag> },{ title: t('lastLogin'), dataIndex: 'lastLoginAt', render: (value?: string) => formatDateTime(value, i18n.language, timezone) },{ title: t('createdAt'), dataIndex: 'createdAt', render: (value: string) => formatDateTime(value, i18n.language, timezone) },{ title: '', render: (_: unknown, item: User) => <Button type="text" aria-label={`${t('edit')} ${item.displayName}`} title={t('edit')} icon={<EditOutlined />} onClick={() => show(item)} /> }]} /></Card>
