...
import { imageArtifactMatchesTemplate, imageRegistryHost, registryMatchesTemplate, templateImageReferences } from '../lib/image-source'
...
export function InstancesPage() {
  ...
  useEffect(() => {
    ...
    setDrawer(true)
  }, [addRequiredHost, canOperate, createRequested, form, hasOnlineHost, loading, requestedImageAvailable, requestedImageID, requestedTemplateAvailable, requestedTemplateID, setParams])
  ...
  const compatibleImages = useMemo(() => images.filter((item) => item.status === 'ready' && !!selected && imageArtifactMatchesTemplate(item.imageRefs, selected.version) && item.architectures.some((architecture) => selected.version.architectures.includes(architecture))), [images, selected])
  ...
  return <><PageHeader title={t('instances')} description={t('instancesDescription')} actions={<><Button icon={<ReloadOutlined />} onClick={() => void load()}>{t('refresh')}</Button>{canOperate && <Button type="primary" loading={loading} icon={hasOnlineHost ? <PlusOutlined /> : <CloudServerOutlined />} onClick={openCreate}>{hasOnlineHost ? t('createInstance') : t('addHost')}</Button>}</>} />
...
export function InstanceDetailPage() {
  ...
  const metricData = metrics.map((metric) => ({ ...metric, time: formatCompactDateTime(metric.collectedAt, i18n.language, timezone) }))
