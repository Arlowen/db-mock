...
import { Alert, App, AutoComplete, Button, Card, Col, Descriptions, Drawer, Dropdown, Form, Grid, Input, InputNumber, Modal, Progress, Radio, Row, Select, Space, Steps, Switch, Table, Tabs, Tag, Typography } from 'antd'
...
export function InstancesPage() {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue(source
      ? deploymentCopyDraft(source, projects.map((project) => project.id))
  ...
  useEffect(() => {
    ...
    if (copySource && selected.version.id === copySource.templateVersionId && !copyPrefillApplied.current) {
      ...
      form.setFieldsValue(deploymentCopyDraft(copySource, projects.map((project) => project.id)))
      return
    }
  ...
  }, [copySource, form, projects, selected, selectedResourceProfiles, selectedTemplateParameters])
  useEffect(() => {
    if (!selected) return
  ...
  const openCreate = () => { if (!hasOnlineHost) { addRequiredHost(); return } copyPrefillApplied.current = false; setCopySource(undefined); setDrawer(true); setStep(0); setCreateError(''); setCreateDraftDirty(false); form.resetFields(); form.setFieldsValue({ environment: 'development', bindAddress: '0.0.0.0', autoRestart: true, imageSource: 'public', projectId: projectFilter || undefined }) }
  ...
  return <><PageHeader title={t('instances')} description={t('instancesDescription')} />
    ...
    {(items.length > 0 || !loadError) && <Card className="instance-table-card" title={!showFilters ? t('instances') : undefined} extra={!showFilters ? listActions : undefined}><Table rowKey="id" loading={loading} dataSource={filteredItems} columns={columns} showHeader={!compactLayout} scroll={compactLayout ? undefined : { x: 968 }} pagination={{ current: page, pageSize, showSizeChanger: !compactLayout, pageSizeOptions: [20, 50], onChange: (nextPage, nextPageSize) => { setPage(nextPageSize === pageSize ? nextPage : 1); setPageSize(nextPageSize) } }} locale={{ emptyText: <EmptyState compact action={emptyAction} actionLabel={emptyActionLabel} description={emptyDescription} /> }} /></Card>}
