...
export function InstancesPage() {
  const { t } = useTranslation(); const { message } = App.useApp(); const navigate = useNavigate(); const notifyTask = useTaskNotification(); const [params, setParams] = useSearchParams(); const [items, setItems] = useState<Instance[]>([]); const [templates, setTemplates] = useState<DatabaseTemplate[]>([]); const [hosts, setHosts] = useState<Host[]>([]); const [projects, setProjects] = useState<Project[]>([]); const [images, setImages] = useState<ImageArtifact[]>([]); const [registries, setRegistries] = useState<Registry[]>([]); const [loading, setLoading] = useState(true); const [creating, setCreating] = useState(false); const [createError, setCreateError] = useState(''); const [actioning, setActioning] = useState(''); const [drawer, setDrawer] = useState(false); const [step, setStep] = useState(0); const [search, setSearch] = useState(''); const [projectFilter, setProjectFilter] = useState(''); const [environmentFilter, setEnvironmentFilter] = useState(''); const [statusFilter, setStatusFilter] = useState(''); const [form] = Form.useForm<CreateValues>()
  ...
  const createIntent = useCallback(() => {
    ...
    return `/instances?create=1${template ? `&template=${encodeURIComponent(template)}` : ''}`
  }, [params])
  ...
  useEffect(() => {
    if (loading || params.get('create') !== '1') return
    if (!hasOnlineHost) { addRequiredHost(); return }
    setStep(0)
    setCreateError('')
    form.resetFields()
    form.setFieldsValue({ environment: 'development', bindAddress: '0.0.0.0', autoRestart: true, imageSource: 'public', templateVersionId: params.get('template') || undefined })
    setDrawer(true)
  }, [addRequiredHost, form, hasOnlineHost, loading, params])
  ...
  const openCreate = () => { if (!hasOnlineHost) { addRequiredHost(); return } setDrawer(true); setStep(0); setCreateError(''); form.resetFields(); form.setFieldsValue({ environment: 'development', bindAddress: '0.0.0.0', autoRestart: true, imageSource: 'public' }) }
  const close = () => { setDrawer(false); setParams({}); setStep(0); setCreateError('') }
  const next = async () => {
  ...
  const quickAction = async (item: Instance, action: string) => { const key = `${item.id}:${action}`; try { setActioning(key); const task = await api<Task>(`/instances/${item.id}/actions/${action}`, { method: 'POST', body: {} }); notifyTask(task); await load() } catch (e) { message.error(errorMessage(e)) } finally { setActioning('') } }
  const columns = [
    ...
    { title: t('template'), render: (_: unknown, item: Instance) => <Space><DatabaseIcon slug={item.templateSlug} name={item.templateName} size="small" /><span>{item.templateName} <Typography.Text type="secondary">{item.templateVersion}</Typography.Text></span></Space> },
    ...
    { title: t('environment'), dataIndex: 'environment', render: (value: string) => <Tag>{translateCode(t, value)}</Tag> },
    { title: '', align: 'right' as const, render: (_: unknown, item: Instance) => { const shouldStop = item.status === 'running' || item.status === 'degraded'; const allowed = shouldStop || item.status === 'stopped' || item.status === 'failed'; const action = shouldStop ? 'stop' : 'start'; const key = `${item.id}:${action}`; return <Space><Button type="text" loading={actioning === key} disabled={!allowed || (!!actioning && actioning !== key)} aria-label={t(action)} title={t(action)} icon={shouldStop ? <PauseCircleOutlined /> : <PlayCircleOutlined />} onClick={() => void quickAction(item, action)} /><Button type="text" aria-label={t('details')} title={t('details')} icon={<MoreOutlined />} onClick={() => navigate(`/instances/${item.id}`)} /></Space> } },
  ]
  const versionOptions = templates.flatMap((item) => item.versions.map((version) => ({ value: version.id, searchText: `${item.name} ${item.nameZh} ${version.version}`, label: <Space><DatabaseIcon slug={item.slug} name={item.name} size="small" /><span>{item.name} {version.version}</span></Space>, template: item, version })))
...
export function InstanceDetailPage() {
  ...
  const failedTask = item.status === 'failed' ? tasks.find((task) => ['failed', 'interrupted', 'canceled'].includes(task.status)) : undefined
  ...
  const operationPanel = operationTask && <div className={`instance-operation is-${activeTask ? 'active' : 'failed'}`}>
    <div className="instance-operation-copy">
      ...
      <Typography.Paragraph type={activeTask ? 'secondary' : 'danger'}>{activeTask ? translateCode(t, operationTask.message, 'taskMessage') : operationTask.errorCode ? translateCode(t, operationTask.errorCode, 'taskError') : operationTask.errorMessage || translateCode(t, operationTask.message, 'taskMessage')}</Typography.Paragraph>
    </div>
  ...
  const canStopOrRestart = item.status === 'running' || item.status === 'degraded'
  const canUpgrade = ['running', 'stopped', 'degraded'].includes(item.status)
  ...
  return <><PageHeader title={<Space><Button type="text" aria-label={t('instances')} title={t('instances')} icon={<LeftOutlined />} onClick={() => navigate('/instances')} />{item.name}<StatusTag value={item.status} /></Space>} description={`${item.templateName} ${item.templateVersion} · ${item.hostName}`} actions={<><Button icon={<EditOutlined />} disabled={!!actioning} onClick={showEdit}>{t('edit')}</Button>{canStart && <Button type="primary" icon={<PlayCircleOutlined />} loading={actioning === 'start'} disabled={!!actioning && actioning !== 'start'} onClick={() => void run('start')}>{t('start')}</Button>}{canStopOrRestart && <Button icon={<PauseCircleOutlined />} loading={actioning === 'stop'} disabled={!!actioning && actioning !== 'stop'} onClick={() => void run('stop')}>{t('stop')}</Button>}{canStopOrRestart && <Button icon={<ReloadOutlined />} loading={actioning === 'restart'} disabled={!!actioning && actioning !== 'restart'} onClick={() => void run('restart')}>{t('restart')}</Button>}<Dropdown menu={{ items: moreActions, onClick: ({ key }) => key === 'upgrade' ? showUpgrade() : showDelete() }} trigger={['click']}><Button icon={<MoreOutlined />} disabled={!!actioning}>{t('moreActions')}</Button></Dropdown></>} />{pageError && <Alert className="instance-page-alert" type="warning" showIcon message={t('instanceRefreshFailed')} description={pageError} action={<Button size="small" onClick={() => void load()}>{t('retry')}</Button>} />}{operationPanel}<Tabs activeKey={activeTab} onChange={changeTab} items={[{ key: 'overview', label: t('details'), children: overview },{ key: 'connection', label: t('connection'), children: connectionTab },{ key: 'logs', label: t('logs'), children: logsTab },{ key: 'metrics', label: t('metrics'), children: metricsTab }]} />
    ...
    <Modal title={`${t('delete')} ${item.name}`} open={deleteOpen} onCancel={() => { if (!actioning) { setDeleteOpen(false); setConfirm('') } }} onOk={() => void run('delete', { confirmName: confirm })} confirmLoading={actioning === 'delete'} okButtonProps={{ danger: true, disabled: confirm !== item.name }}><Typography.Paragraph>{t('dangerConfirm')}</Typography.Paragraph><Input aria-label={t('dangerConfirm')} value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder={item.name} /></Modal>
