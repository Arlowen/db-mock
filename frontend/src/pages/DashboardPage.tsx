...
export function DashboardPage() {
  ...
  const cards = [
    ...
    { title: t('alerts'), value: data.openAlerts, suffix: 'open', icon: <AlertOutlined />, color: '#fa8c16' },
  ]
  return <><PageHeader title={t('dashboard')} description="Linux & macOS Docker Compose control plane" actions={<Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/instances?create=1')}>{t('createInstance')}</Button>} />
    ...
    <Row gutter={[16, 16]} className="dashboard-grid"><Col xs={24} xl={12}><Card title={t('instances')} extra={<Button type="link" onClick={() => navigate('/instances')}>View all</Button>}><List dataSource={instances} locale={{ emptyText: t('noData') }} renderItem={(item) => <List.Item onClick={() => navigate(`/instances/${item.id}`)} className="clickable-list"><List.Item.Meta avatar={<span className="template-avatar">{item.templateSlug.slice(0, 2).toUpperCase()}</span>} title={item.name} description={`${item.templateName} ${item.templateVersion} · ${item.hostName}`} /><StatusTag value={item.status} /></List.Item>} /></Card></Col>
    <Col xs={24} xl={12}><Card title={t('tasks')}><List dataSource={tasks} locale={{ emptyText: t('noData') }} renderItem={(item) => <List.Item onClick={() => navigate('/tasks')} className="clickable-list"><List.Item.Meta title={item.kind} description={item.message || item.stage} /><StatusTag value={item.status} /></List.Item>} /></Card></Col>
