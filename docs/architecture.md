...
项目部署默认值存储在项目元数据中，仅包含受限枚举的环境、0–365 天的使用周期和字符串标签。
项目接口沿用管理员/运维写权限并记录默认值变更审计；只读角色可查看。前端在普通创建时将默认值
转换为标准实例创建字段，服务端仍执行现有权限、模板、容量、端口和主机在线校验。凭据、目标主机、
资源规格、数据库初始化参数和额外环境变量不属于项目默认值，避免跨模板或跨主机复用失效配置。
