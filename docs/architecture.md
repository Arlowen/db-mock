...
批量生命周期入口调用受运维权限保护的 `POST /api/v1/instances/batch-actions/{start|stop}`，
...
返回 accepted/rejected 列表，前端只对已停止实例批量启动、只对运行中或异常实例批量停止，并将
其他选中项在确认前标记为跳过。批量删除不复用该入口，仍保留逐实例名称确认。
