...
- Run checks proportional to the change before pushing. For frontend changes, run at least `npm run typecheck`, `npm test`, and `npm run build`; for backend changes, run `cd backend && go test ./...`.
